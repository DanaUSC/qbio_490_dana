...
clin_query <- #Fill in correct command
  #I ADDED IN DIFFERENT CODE BLOCK BC OF TECHNOLOGICAL CHALLENGES and having to come back to this part.
...
clinic <- read.csv("/Users/danasouter/Desktop/qbio490/qbio_490_dana/analysis_data/skcm_clinical_data.csv")
```
...
# 1. How many rows are there? How many columns?  470 rows and 65 columns
# 2. What is the data type of $gender? chr
# 3. What is the age in years of the 5th patient? 74 years
# 4. How old was the 2nd patient at their last follow up? HINT: you need to look at data from two columns to answer this. 82 years.

```{r Exercise 3.1}
# write code here

#age_5th_patient <- clinic$age[5]
#age_5th_patient
...
```{r Exercise 4.1}
# create plot here
plot(clinic$weight_kg_at_diagnosis, clinic$height_cm_at_diagnosis)
```
...
```{r Exercise 4.2}
# create plot here
#boxplot(clinic$gender, clinic$age_at_diagnosis)
...
```{r}
vital_status_na_mask <- complete.cases(clinic$vital_status)
clinic_clean <- clinic[vital_status_na_mask, ]
...
```{r}
cleaned_clinic$survival_time <- ifelse(cleaned_clinic$death_days_to == "[Not Available]", cleaned_clinic$last_contact_days_to, cleaned_clinic$death_days_to)
...
cleaned_clinic$survival_time <- ifelse(cleaned_clinic$death_days_to == "[Completed]", cleaned_clinic$last_contact_days_to, cleaned_clinic$death_days_to)
...
```{r}
cleaned_clinic$survival_time <-cleaned_clinic$survival_time <- as.numeric(gsub("\\[|\\]", "", cleaned_clinic$survival_time))

  #as.numeric(cleaned_clinic$survival_time)
```
...
A p-value is the probability value to determine the significance of results. A statistically significant association between age and skin cancer survival outcomes.
